...
class NaverService:
    ...
    @staticmethod
    async def search_blogs(keyword: str, start: int = 1, display: int = 10) -> Dict[str, Any]:
        """
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    raise Exception(f"네이버 API 호출 실패: {response.status}, {error_text}")
